...
class Calibrations:
    ...
    def load_data(self):
        ...
        #deployment_url_v2 = "https://api.dexcom.com/v2/users/self/calibrations"
        #deployment_url_v3 = "https://api.dexcom.com/v3/users/self/calibrations"
        headers = {"Authorization": f"Bearer {access_token}"}
        params = {'startDate': self.start_date, 'endDate': self.end_date}

        try:
            response = requests.get(url, headers=headers, params=params)
            #response = requests.get(deployment_url_v2, headers=headers, params=params)
